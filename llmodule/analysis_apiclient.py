...
try:
    ...
    # issues, e.g. raising ConfigParser exceptions that configparser cannot catch.
    # To avoid this we continue to use ConfigParser on python 2 and configparser on python 3.
    # Note that usages of configparser must be forward compatible with python 3's configparser,
    # in order to maintain cross-compatibility between python 2 and python 3.
...
class NamedStringIO(io.BytesIO):
    """
    A wrapper around BytesIO to make it look more like a real file-stream.
    This class was introduced to give a stream the concept of a name, which the download provides.
    However, it does intentionally break backwards compatibility by returning bytes instead of str
    ...
    def name(self):
        """
        Get the name of the stream, might be None
        """
        return self._name
...
class TaskCompletion(object):
    ...
    def get_completed(self, after, before=None):
        """
        ...
        :param before: Request tasks completed before this time. If None specified, defaults to
            current time on api server
        :type before: datetime.datetime | None
        ...
        try:
            while True:
                ...
                if not more:
                    break
                last_ts = parse_datetime(data["resume"])
                if before is not None and last_ts >= before:
                    break
                after = last_ts
...
class AnalysisClientBase(object):
    ...
    def _str_to_bytesio(self, base_str):
        """
        Given a base string, convert and return BytesIO|None

        :param str|bytes|unicode base_str: base string to convert to bytes
        :return: converted bytes
        :rtype: io.BytesIO
        :raises TypeError: If the input is something other than str|bytes|unicode|None.
        :raises UnicodeDecodeError: When the input cannot be encoded as utf-8
        """
